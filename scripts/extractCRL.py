...
def main( filename ):
    ...
    if os.path.exists( "masterListCRL.pem" ):
        os.remove( "masterListCRL.pem" )
...
def extractPEMCertificateV2( der ):
    global totalCerts
    try:
        (x509,err) = execute( f"openssl x509 -inform der -outform pem", der )
        # print( "Verify certificate from payload, err", err.decode("utf8").split("\n") )
        # print( "Verify certificate from payload, x509", x509.decode("utf8").split("\n") )
        totalCerts += 1
        # Append to masterListCRL.pem
        with open("masterListCRL.pem", "ab") as f:
            f.write(x509)
    except Exception as e:
        print( "Verify certificate from payload - execute, e", e )
